...
@export var turn_speed_deg: float = 660.0 # vitesse de rotation du FOV (deg/sec)

#distance_to_target to use for fleeing or on a different class for kitting
#maybe use composition for behaviors ?
...
enum STATE { IDLE, RETURNING, ROAMING, PATROLLING, CHASING, ATTACKING, SEARCHING, FLEEING, DEAD }
...
##### @todo rotate l'enemy vers moi parce que sinon juste je quitte le champs de vision et cest fini quoi
### Le souci c'est que dans mon autre jeu je faisais du calcul à la mano donc il pouvait gérer, plus maintenant
### Estce que c'est mieux de faire comme l'autre jeu ? Mais du coup tu vois sur une distance de ouf 'fin niveau calcul etc c'est pas trop lourd
...
func update_action_by_state(delta: float):
	match state:
		STATE.IDLE:
			pass
		STATE.RETURNING:
...
func process_fleeing(delta):
	# @todo add if attacking can't move ?
...
func _update_facing(delta: float) -> void:
	...
	# 1. Combat, turns to playerd
	if (state == STATE.CHASING or state == STATE.ATTACKING) and _player_in_fov and _check_raycast_to_player():
		_rotate_node_towards(sprite_container, _player_ref.global_position, delta)
		return
	
	# 2. Search turns to last_seen_pos
	if state == STATE.SEARCHING:
		_rotate_node_towards(sprite_container, _last_seen_pos, delta)
		return
	
	if state == STATE.RETURNING:
		_rotate_node_towards(sprite_container, _initial_position, delta)
		return
	...
	if state == STATE.PATROLLING and _waypoints.size() > 0:
		...
		_rotate_node_towards(sprite_container, target, delta)
		return
	
	# 4. Idle / return → keep initial rotation
	if state == STATE.IDLE:
		_rotate_node_towards(sprite_container, global_position + _initial_facing_direction, delta)
